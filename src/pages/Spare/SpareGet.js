...
class SpareGet extends React.Component {
  ...
  handleSearchc = (selectedKeys, dataIndex, end) => {
    ...
    if (end) {
      let start = dataIndex;
      this.setState({ postDatac: { ...this.state.postDatac, [start]: selectedKeys[0] ? selectedKeys[0] : "", [end]: selectedKeys[1] ? selectedKeys[1] : "" } }, () => {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'status',
        key: 'status',
        ...getselectbox("status", [{dicKey:"0",dicName:"待审批"},
      {dicKey:"1",dicName:"审批通过"},{dicKey:"2",dicName:"审批未通过"},{dicKey:"3",dicName:"撤回"},
      ]),
        render: (text, record) => {
          let name = text == 0 ? "待审批" :
            text == 1 ? "审批通过" :
    ...
    this.columns = [
      ...
      },
      {
        ...
        dataIndex: 'sparePartsTypeName',
        key: 'sparePartsTypeName',
        ...gettreeselectboxc( "sparePartsTypeId" , this.props.publicmodel.dataList )
      },
      {
        title: '可用库存',
        dataIndex: 'availableStock',
    ...
    const expandedRowRender = (record) => {
      ...
      return <Table bordered size="middle" columns={columnss} dataSource={record.sparePartsApplyAuditList} pagination={false} />;
    };
