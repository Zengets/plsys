...
class Child extends React.Component {
    ...
    outputCur(){
        let { postUrl, postData } = this.state;
        this.setNewState("aexportFileCheck", {
            shopId: postData.shopId,
            companyId:postData.companyId,
            planMonth:postData.planMonth
        }, () => {
            message.loading("正在导出文件...")
            window.open(`/rs/shopMonthFinishReport/exportFile?shopId=${postData.shopId?postData.shopId:""}&companyId=${postData.companyId?postData.companyId:""}&planMonth=${postData.planMonth?postData.planMonth:""}`)
        })
    }
