...
@connect(({ approval, loading }) => ({
    approval,
    submitting: loading.effects['approval/buyqueryList'],
}))
class SpareBuyChild extends React.Component {
    ...
    render() {
        let { postData, postUrl, fv, fields, iftype, curitem } = this.state,
            { buyqueryList, nodeList } = this.props.approval;
                    ...
                    this.props.postData.status == "1" ? "审批通过列表" :
                        this.props.postData.status == "2" ? "审批未通过列表" :
                            this.props.postData.status == "3" ? "撤回列表" : ""} extra={
                                <div>
                                    {
                                        curitem.id&&this.props.postData.status == "0"? <span>
                                            <a onClick={() => {
                                                this.setState({
                                                    iftype: {
                                                        name: "审批",
                                            ...
                                            <Popconfirm
                                                ...
                                                onConfirm={() => {
                                                    this.setNewState("buyrecall", { id: curitem.id }, () => {
                                                        let total = this.props.approval.buyqueryList.total,
                                                            page = this.props.approval.buyqueryList.pageNum;
                                                        if ((total - 1) % 10 == 0) {
                                                            page = page - 1
                                                        }
                                                        this.setState({
