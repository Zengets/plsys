...
class StateLine extends Component {
  ...
  render() {
    let { totals, lists, queryShopsEquipChartLine } = this.props.home,
      { } = this.state,
      { equipmentCount, turnOnRate, equipStatusChart, name, actualTurnOnCount, planTurnOnCount, actualTurnOnRate } = totals;
    ...
    let col = {
      ...
      xs: 24, sm: 24, md: 24, lg: 24, xl: 24, xxl: 12,
    }, coles = {
    ...
    return (
      <Spin spinning={this.props.submitting && this.props.submittings} tip="数据加载中...">
                    ...
                    <Card title={<span title={item.name + "(" + item.companyName + ")"}>{item.name + "(" + item.companyName + ")"}</span>} >
                      <Row>
                        <Col {...coles} style={{ padding: 6 }}>
                          <Statistic title="产线总数" value={item.equipmentCount} prefix={<Icon type="desktop" />} />
                        </Col>
                        <Col {...coles} style={{ padding: 6 }}>
                          <Statistic title="开机率" value={item.turnOnRate} prefix={<Icon type="stock" />} suffix="%" />
                        </Col>
                        <Col {...coles} style={{ padding: 6 }}>
                          <Statistic title="计划开线数" value={item.planTurnOnCount} prefix={<Icon type="file" />} suffix="台" />
                        </Col>
                        <Col {...coles} style={{ padding: 6 }}>
                          <Statistic title="实际开线数" value={item.actualTurnOnCount} prefix={<Icon type="bulb" />} suffix="台" />
                        </Col>
                        <Col {...coles} style={{ padding: 6 }}>
                          <Statistic title="实际开线率" value={item.actualTurnOnRate} prefix={<Icon type="bar-chart" />} suffix="%" />
                        </Col>
                      </Row>
        ...
        <Row gutter={12}>
              ...
              <Row>
                <Col {...cold} style={{ margin: "12px 0px" }}>
                  <ChartCard
                    bordered={false}
                    title="当前产线总数"
                    total={() => <span style={{ fontSize: 18 }}><a style={{ color: "#f2637b", fontSize: 16 }}>{equipmentCount ? parseInt(equipmentCount) : 0}</a> 台</span>}
                  />
                </Col>
                <Col {...cold} style={{ margin: "12px 0px" }}>
                  <ChartCard
                    bordered={false}
                    title="产线开机率"
                    total={() => <span style={{ fontSize: 18 }}><a style={{ color: "#2997ff", fontSize: 16 }}>{turnOnRate ? parseFloat(turnOnRate).toFixed(2) : 0}</a> %</span>}
                  />
                </Col>
                <Col {...cold} style={{ margin: "12px 0px" }}>
                  <ChartCard
                    bordered={false}
                    title="计划开线数"
                    total={() => <span style={{ fontSize: 18 }}><a style={{ color: "#2997ff", fontSize: 16 }}>{planTurnOnCount ? planTurnOnCount : 0}</a> 台</span>}
                  />
                </Col>
                <Col {...cold} style={{ margin: "12px 0px" }}>
                  <ChartCard
                    bordered={false}
                    title="实际开线数"
                    total={() => <span style={{ fontSize: 18 }}><a style={{ color: "#2997ff", fontSize: 16 }}>{actualTurnOnCount ? actualTurnOnCount : 0}</a> 台</span>}
                  />
                </Col>
                <Col {...cold} style={{ margin: "12px 0px" }}>
                  <ChartCard
                    bordered={false}
                    title="实际开线率"
                    total={() => <span style={{ fontSize: 18 }}><a style={{ color: "#2997ff", fontSize: 16 }}>{actualTurnOnRate ? actualTurnOnRate : 0}</a> %</span>}
                  />
                </Col>
                      ...
                      equipStatusChart ? equipStatusChart.map((item, i) => {
                        return <li className={styles.hoverables} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }} key={i} onClick={() => {
                          ...
                          <span style={{ color: getcolor(item.name) }}>
                            <i style={{ padding: 2, color: getcolor(item.name) }}></i> {item.name}</span>
                          <span style={{ color: getcolor(item.name) }}>{item.value}</span>
                          <div className={styles.bacline} style={{ width: `${parseFloat(item.value * 100 / getallNum(equipStatusChart, "value"))}%`, backgroundColor: getcolor(item.name), opacity: 0.5 }}>
                          </div>
                        </li>
                      }) : null
                    }
          ...
          <Col {...cols}>
            <Card title={"详情"} extra={lists.length > 0 ?
              lists[0].id ? null :
                <a onClick={() => {
                  this.resetData();
                }}>查看汇总</a> :
                ...
                lists && lists.length > 0 ?
                  lists.map((item, i) => (
                    <Col {...colc} key={i} style={{ marginBottom: 12 }}>
                      <Card title={<span title={item.name}>{item.name}</span>} extra={item.id ? <a onClick={() => {
                        item.id ?
                          this.setNewState("queryEquipChartByCompanyIdLine", { companyId: item.id }) :
                          null
                        ...
                        this.setState({
                          ...
                          this.setNewState("queryShopsEquipChartLine", { shopName: item.name }, () => {
