...
class RepairDuty extends React.Component {
  ...
  render() {
    let { postData, postUrl, fv, fields, iftype, curitem, charge, selectedRowKeys, selectedRowKeyc, postDatas, postDatac } = this.state,
      { queryBusinessByUserId, equipmentqueryList, userList, uiassociation, equipmentqueryNoByUserId, equipmentqueryByUserId, deviceTypequeryTreeList, companyList } = this.props.repair;
    const rowSelections = {
                    ...
                    onChange={(val) => {
                      this.setState({
                        ...
                        this.setNewState("queryUpdateByUserId", {
                          "userId": val,   //------------原负责人id*
                          "chargeType": "3"    //----------------负责类型(0;点检   1:保养    2:验证     3:维修)*
                        },()=>{
                          this.setState({
